...
def test_skgbr(X_train, y_train, X_test, y_test):
    model = SKGBR(n_estimators=200, max_depth=5, max_features=0.2, learning_rate=0.05, max_leaf_nodes=50)
...
def test_skrf(X_train, y_train, X_test, y_test):
    model = SKRF(n_estimators=200, max_depth=5, max_features=0.4, max_leaf_nodes=1000)
    model.fit(X_train, y_train)
    predictions = model.predict(X_test)
    mse = regression.mean_squared_error(y_test, predictions)
    print 'mse:{}'.format(mse)
...
def test_gbr(X_train, y_train, X_test, y_test):
    model = GradientBoostingRegressor(n_estimators=200, max_depth=3, max_features=None, learning_rate=0.1)
    model.fit(X_train, y_train)
    predictions = model.predict(X_test)
    mse = regression.mean_squared_error(y_test, predictions)
    print 'mse:{}'.format(mse)
...
def test_rf(X_train, y_train, X_test, y_test):
    model = RandomForestRegressor(n_estimators=200, max_features=None, min_samples_split=25, max_depth=5, criterion='mse')
    model.fit(X_train, y_train)
    predictions = model.predict(X_test)
    mse = regression.mean_squared_error(y_test, predictions)
    print 'mse:{}'.format(mse)
...
def test_dart(X_train, y_train, X_test, y_test):
    model = DARTRegressor(n_estimators=200, max_depth=X_train.shape[0] * 0.4, min_samples_split=25, p=0.01)
    model.fit(X_train, y_train)
    predictions = model.predict(X_test)
    mse = regression.mean_squared_error(y_test, predictions)
    print 'mse:{}'.format(mse)
...
def test_pdart(X_train, y_train, X_test, y_test):
    model = pDARTRegressor(n_estimators=200, max_features=0.4, max_depth=5, max_leaf_nodes=50)
    model.fit(X_train, y_train)
    predictions = model.predict(X_test)
    mse = regression.mean_squared_error(y_test, predictions)
    print 'mse:{}'.format(mse)
