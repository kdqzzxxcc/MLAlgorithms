...
class LambdaMART:
    def __init__(self, training_data=None, n_estimators=100, learning_rate=1., max_features=None, max_depth=2, min_samples_split=10,
    ...
    def fit(self):
        ...
        for k in xrange(self.number_of_trees):
            print 'Tree %d, at %s' % (k, time.ctime())
            ...
            for lambda_val, w_val, query_key in pool.map(compute_lambda,
                ...
                w[indexes] = w_val
            pool.close()
            print 'Compute {} lambda success, at {}'.format(k, time.ctime())
            ...
            tree = DecisionTreeRegressor(criterion='friedman_mse', splitter="best", max_depth=self.max_depth,
                                         max_features=self.max_feature, max_leaf_nodes=self.max_leaf_nodes,
    ...
    def predict(self, data):
        ...
        predicted_scores = np.zeros(data.shape[0])
    ...
    def predict_all(self, data):
        ...
        for tree in self.trees:
            predictions += self.learning_rate * tree.predict(data)
        return predictions
